<template>
  <WizardForm :form="form" @on-validate="enviar">
    ...
    <FormItem label="CPF Responsável (Administrador do Sistema)" prop="cpF_Responsavel">
    ...
    <FormItem label="idCloud" prop="idCloud">
      <Select v-model="form.fields.idCloud" filterable>
        <Option v-for="(cloud, indexCloud) in cloud.data" :value="cloud.value" :key="indexCloud">{{ cloud.label }}
        </Option>
      </Select>
    </FormItem>
    ...
    <FormItem label="Unidade Espaço Disco" prop="uniEspacoDisco">
      <Select v-model="form.fields.uniEspacoDisco">
        <Option v-for="(unidade, i) in unidadeDisco.data" :key="unidade.value" :value="unidade.value">{{
    unidade.label }}
        </Option>
      </Select>
    </FormItem>
    ...
    <FormItem label="Armazenamento em Meses para arquivos com Casos/Ocorrências" prop="armazenamentoMesesCriticos">
...
export default {
  ...
  watch: {
    ...
    "unidadeDisco.originalData"(to) {
      this.unidadeDisco.data = [
        { value: 0, label: 'Escolha uma opção' },
        ...to.map(el => ({
          value: el.id,
          label: el.descricao,
        }))
  ...
  methods: {
    async initData() {
      ...
      await get("/api/Auxiliar/UnidadeDisco").then(
        (data) => (this.unidadeDisco.originalData = data)
      );
    },
